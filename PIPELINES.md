...
```bash
filesrc location=input.mp4 ! qtdemux ! h264parse ! vpudec_h264 ! imxvideoconvert_g2d ! video/x-raw,format=NV12
```
...
```bash
vpuenc_h264 bitrate=3000 gop-size=30 ! h264parse ! rtph264pay config-interval=1 pt=96 ! udpsink host=192.168.1.100 port=5000
```
...
gst-launch-1.0 udpsrc port=5000 caps="application/x-rtp,media=video,encoding-name=H264,clock-rate=90000,payload=96" ! \
    rtph264depay ! h264parse ! vpudec_h264 ! imxvideoconvert_g2d ! kmssink
```
